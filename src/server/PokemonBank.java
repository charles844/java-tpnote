...
// chargement du pokemon stoke dans la database
	...
	public void handleClient() throws IOException, ClassNotFoundException {
		 ...
		 */
		Socket client=server.accept();//acceptation de la connection
		 ...
		 * - BankOperation
		 */
		ObjectInputStream inputstr=new ObjectInputStream(client.getInputStream());//flux lecture
		ObjectOutputStream outputstr=new ObjectOutputStream(client.getOutputStream());//flux ecriture
		...
		while (running) {
			 ...
			 */
			Request request;
			request= (Request) inputstr.readObject();
			...
			case LIST:
				...
				if (this.pokemons.size() == 0) {
					 ...
					 */
					outputstr.writeObject("No pokemon in database.");// si la taille est zero, il ny a pas de pokemon
                    outputstr.flush();
			...
			case STORE:
                ...
                pokemons.add((Pokemon) inputstr.readObject());
                outputstr.flush();
				 ...
				 */
                outputstr.flush();
                outputstr.writeObject("Pokemon safe.");
